...
do
  python pretraining/run_singlelang_pretraining.py \
    ...
    experiment_name=subset_analysis_bc1 group_name=${lang}_wiki run_name=mlm-tw_${lang}_${n}_${chunk} check_git_status=True use_wandb=True \
    ...
    dev=True data_subset=["mlm_tgt.${lang}","wiki.${lang}"] chunk_dataset=${chunk} training_arguments.num_train_epochs=80 model=bert-base-multilingual-cased use_first_n=${n}
