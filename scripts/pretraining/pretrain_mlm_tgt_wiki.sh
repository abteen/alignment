...
python pretraining/run_singlelang_pretraining.py \
  ...
  experiment_name=alignment_pretraining group_name=mlm_tgt_wiki_80 run_name=mlm_tgt_wiki_80.${lang} check_git_status=True use_wandb=True \
  ...
  dev=False data_subset=["mlm_tgt.${lang}","wiki.${lang}"] chunk_dataset=True training_arguments.num_train_epochs=80 model=${model}