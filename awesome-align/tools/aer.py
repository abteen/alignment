...
if __name__ == "__main__":
    ...
    print("{0}: {1:.2f}% ({2:.2f}%/{3:.2f}%/{4})".format(args.hypothesis,
                aer * 100.0, precision * 100.0, recall * 100.0, sum([len(x) for x in hypothesis])))
    if args.fAlpha >= 0.0:
        print("F-Measure: {:.3f}".format(f_measure))

    if args.log_directory:
        model_info = args.hypothesis
        info_splits = [x for x in args.hypothesis.split('/') if x]
        model = info_splits[-2]
        lang = info_splits[-1].replace('.output', '')

        if args.log_file_name:
            model = args.log_file_name

        with open(os.path.join(args.log_directory, 'output_{}.log'.format(model)), 'a') as out_f:
            out_f.write('Model:\t{0}\tLang/Split:\t{1}\taer:\t{2:.2f}\tprec:\t{3:.1f} recall:\t{4:.1f}\tn\t{5}\tF_measure\t{6}\n'.format(
                model,
                lang,
